// src/components/SalesOrderDeskDashboard.jsx
import React, { useState, useEffect } from 'react';
import '../styles/Dashboard.css';
import logo from '../assets/Serviceman.jpeg';

function SalesOrderDeskDashboard() {
  ...
  const fetchQueue = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/customers/sales-order-desk?companyId=${companyId}` // ✅ ADD COMPANY FILTER
      );
  ...
  const handlePullCustomer = async (customerToServe) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/customers/${customerToServe.id}/start-service?servicePoint=SALES_ORDER_DESK`,
        { method: 'POST' }
      );
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const completeResponse = await fetch(
        `${API_BASE_URL}/customers/${currentCustomer.id}/complete-service?servicePoint=SALES_ORDER_DESK`,
        {
          method: 'POST',
      ...
      if (nextServicePoint === 'Invoicing Desk') {
        nextServicePointCode = 'INVOICING_DESK';
      } else {
        nextServicePointCode = 'GATE_ATTENDANT';
      }

      const forwardResponse = await fetch(
        `${API_BASE_URL}/customers/${currentCustomer.id}/forward?currentServicePoint=SALES_ORDER_DESK&nextServicePoint=${nextServicePointCode}`,
        { method: 'PUT' }
      );
      ...
      <div className="dashboard-container">
        ...
        <div className="dashboard-form-container">
          <h2 className="welcome-message">
            Welcome, <strong>{userName}</strong>! (Sales Order Staff)
          </h2>
...
export default SalesOrderDeskDashboard;